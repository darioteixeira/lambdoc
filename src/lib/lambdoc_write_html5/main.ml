...
struct
	...
	(struct
		...
		let default_valid_options =
			...
			book_lookup = (fun isbn -> Html5.uri_of_string ("isbn:" ^ (Book_output.string_of_isbn isbn)));
			cover_lookup = (fun isbn cover -> Html5.uri_of_string (Book_output.string_of_isbn isbn));
			image_lookup = Html5.uri_of_string;
			namespace = "doc";
			prefix = "doc";
			base_classes = ["valid"];
			extra_classes = [];
		...
		let write_valid ?(valid_options = default_valid_options) doc =
			...
			(********************************************************)
			(* 	{3 Predefined sequences for Q&A}		*)
			(********************************************************)
			...
			(********************************************************)
			(*	{3 Fetch fields from doc record}		*)
			(********************************************************)
			...
			(********************************************************)
			(*	{3 Converters for inline context}		*)
			(********************************************************)
			...
			and write_inline {Inline.inline; attr} =
				...
				| Glyph (alias, alt) ->
					Html5.img ~a:[a_class (!!"glyph" :: attr)] ~src:(opts.image_lookup alias) ~alt ()
			...
			(********************************************************)
			(*	{3 Converters for tabular environment}		*)
			(********************************************************)
			...
			(********************************************************)
			(*	{3 Converters for document blocks}		*)
			(********************************************************)
			...
			and write_block ?(wrapped = false) {block; attr} =
				...
				| Picture (width, alias, alt) ->
					...
					let uri = opts.image_lookup alias in
				...
				| Bookpic (isbn, cover) ->
					...
					let book_uri = opts.book_lookup isbn in
					let cover_uri = opts.cover_lookup isbn cover in
					[Html5.div ~a:[a_class (!!"bookpic" :: attr @ make_floatable wrapped)]
						[
						Html5.a ~a:[a_href book_uri; a_class [!!"pic_lnk"]] [Html5.img ~a:[a_class [!!"pic"]] ~src:cover_uri ~alt ()];
