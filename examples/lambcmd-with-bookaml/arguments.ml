...
let amazon = "OPTIONS: Amazon credentials (only required for book lookups)"
...
let amazon_locale =
    let locale_parser str =
        try `Ok (Bookaml_amazon.Locale.of_string str)
        with _ -> `Error (Printf.sprintf "Unknown Amazon locale '%s'" str) in
    let locale_printer fmt x =
        Format.pp_print_string fmt (Bookaml_amazon.Locale.to_string x) in
    let locale_converter = (locale_parser, locale_printer) in
    let doc = "Amazon Web Services locale" in
    Arg.(value @@ opt (some locale_converter) None @@ info ~docs:amazon ~doc ["amazon-locale"])

let amazon_associate_tag =
    let doc = "Amazon Web Services associate tag" in
    Arg.(value @@ opt (some string) None @@ info ~docs:amazon ~doc ["amazon-associate-tag"])

let amazon_access_key =
    let doc = "Amazon Web Services access key" in
    Arg.(value @@ opt (some string) None @@ info ~docs:amazon ~doc ["amazon-access-key"])

let amazon_secret_key =
    let doc = "Amazon Web Services secret key" in
    Arg.(value @@ opt (some string) None @@ info ~docs:amazon ~doc ["amazon-secret_key"])
...
let build
    ...
    amazon_locale amazon_associate_tag amazon_access_key amazon_secret_key
