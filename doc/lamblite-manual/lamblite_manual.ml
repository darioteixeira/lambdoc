...
let main_handler sp () () =
        let ch = open_in "lamblite_manual.ltex" in
        let src = Std.input_all ch in
        let () = close_in ch in
	let css_uri = Eliom_predefmod.Xhtml.make_uri (Eliom_services.static_dir sp) sp ["css"; "lambdoc.css"] in
	let classnames = ["warn"; "tip"] in
	let accept_list = [] in
	let deny_list = [] in
	let default = `Accept in
	let time1 = Unix.gettimeofday () in
	let doc = Lambdoc_read_lambtex.Main.ambivalent_manuscript_from_string ~classnames ~accept_list ~deny_list ~default src in
	...
	Lwt.return
		(html
			(head ~a:[a_profile (uri_of_string "http://www.w3.org/2005/11/profile")]
				(title (pcdata "The Lamblite Manual"))
				[
